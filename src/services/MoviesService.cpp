...
map<int, int> MoviesService::mapComparableMovies(int userID, vector<int> *users, vector<int> *ranks) {
    Database *db = StateManager::getInstance()->getDb();
    ...
    for (int i: *users) {
        ...
        for (int j: userMovies) {
            //In case the original user has watched the movie, we will not add it to the recommendation map
            if (db->userHasMovie(userID, j)) {
                continue;
            }
